...
export function renderPlaybackTab(): HTMLElement {
  ...
  void window.electron.getSettings().then((settings) => {
    ...
    const UPSCALE_IDS = [8, 9, 10, 11, 12, 13];
    const warnEl = document.createElement('div');
    warnEl.className = 'settings-upscale-warning';
    warnEl.style.display = UPSCALE_IDS.includes(upscaleMode) ? 'flex' : 'none';
    warnEl.innerHTML = `
      <span class="settings-upscale-warning__icon" aria-hidden="true"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m21.73 18-8-14a2 2 0 0 0-3.48 0l-8 14A2 2 0 0 0 4 21h16a2 2 0 0 0 1.73-3Z"/><path d="M12 9v4"/><path d="M12 17h.01"/></svg></span>
    ...
    modeSection.appendChild(warnEl);
